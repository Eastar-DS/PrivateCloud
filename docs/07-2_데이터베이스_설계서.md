...
멀티 테넌트 환경에서 데이터 가용성과 읽기 성능을 보장하기 위해 Primary-Replica 이중화 구성을 적용한다.

| 역할 | 호스트명 | VLAN | IP 대역 | 용도 |
|------|----------|------|---------|------|
| **Primary** | dev-data-db-pri | VLAN 50 | 10.0.50.x | Read / Write |
| **Replica** | dev-data-db-rep | VLAN 50 | 10.0.50.x | Read Only |
...
sesac-smartdocs 데이터베이스는 6개의 테이블로 구성되며, 테넌트(부서) 기반의 멀티 테넌시를 지원한다.
...
│ code     (UQ)│          │ parent_id    (FK)│          │ tenant_id    │
│ quota_bytes  │          │ title            │          │ name         │
│ used_bytes   │          │ file_name        │          │ confidence   │
│ is_active    │          │ sha256_hash      │          │ source       │
│ created_at   │          │ tagging_status   │          │ model_version│
...
| 관계 | 설명 |
|------|------|
| tenants 1:N documents | 테넌트(부서)는 여러 문서를 보유한다 |
...
| documents 1:N documents | 문서 버전 관리를 위한 자기 참조 |
...
업로드된 문서의 메타데이터를 관리하는 핵심 테이블이다. 문서 원본 파일은 오브젝트 스토리지(MinIO)에 저장되며, 이 테이블은 메타데이터만 보관한다.
...
CREATE TABLE documents (
    ...
    INDEX idx_doc_parent (parent_id),
    INDEX idx_doc_hash (sha256_hash),
    FULLTEXT INDEX ft_doc_title (title, description)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
```
...
| mime_type | VARCHAR(100) | NOT NULL | MIME 타입 (예: application/pdf) |
| sha256_hash | CHAR(64) | NOT NULL | 파일 해시값 (중복 검출용) |
| storage_key | VARCHAR(1000) | NOT NULL | 오브젝트 스토리지 경로 |
| version | INT | NOT NULL, DEFAULT 1 | 문서 버전 번호 |
| parent_id | VARCHAR(36) | FK | 이전 버전 문서 ID (버전 관리) |
...
문서에 부여된 태그 정보를 관리하는 테이블이다. AI 자동 태깅 결과 또는 사용자 수동 태깅 결과를 저장한다.
...
| source | ENUM | NOT NULL | 태그 생성 출처 (AI 또는 MANUAL) |
...
| documents | idx_doc_tenant | tenant_id | B-Tree | 테넌트별 문서 목록 |
| documents | idx_doc_status | tagging_status | B-Tree | 태깅 상태별 필터링 |
| documents | idx_doc_parent | parent_id | B-Tree | 문서 버전 체인 조회 |
| documents | idx_doc_hash | sha256_hash | B-Tree | 중복 파일 검출 |
| documents | ft_doc_title | title, description | FULLTEXT | 문서 전문 검색 |
| tags | PRIMARY | id | PK | 기본 키 조회 |
| tags | idx_tags_doc | document_id | B-Tree | 문서별 태그 목록 |
| tags | idx_tags_tenant | tenant_id | B-Tree | 테넌트별 태그 조회 |
...
| tagging_jobs | idx_jobs_tenant | tenant_id | B-Tree | 테넌트별 작업 목록 |
| audit_logs | PRIMARY | id | PK | 기본 키 조회 |
| audit_logs | idx_audit_tenant | tenant_id | B-Tree | 테넌트별 감사 로그 |
...
| 쿼리 패턴 | 사용 인덱스 | 설명 |
|-----------|------------|------|
| 테넌트별 문서 목록 조회 | idx_doc_tenant | WHERE tenant_id = ? AND is_deleted = FALSE |
| 태깅 대기 문서 조회 | idx_doc_status | WHERE tagging_status = 'PENDING' |
| 문서 전문 검색 | ft_doc_title | MATCH(title, description) AGAINST(? IN BOOLEAN MODE) |
...
public List<DocumentDto> findByTenantId(String tenantId) {
    return documentRepository.findByTenantIdAndIsDeletedFalse(tenantId);
}
...
| documents | description | ft_doc_title | 문서 설명 검색 |
...
```sql
-- 문서 제목 및 설명에서 키워드 검색
SELECT d.id, d.title, d.description, d.file_name,
       d.created_at, d.tagging_status,
       MATCH(d.title, d.description) AGAINST(:keyword IN BOOLEAN MODE) AS relevance
  FROM documents d
 WHERE d.tenant_id = :tenantId
   AND d.is_deleted = FALSE
   AND MATCH(d.title, d.description) AGAINST(:keyword IN BOOLEAN MODE)
...
```sql
-- 문서 제목/설명 + 태그 이름 통합 검색
SELECT d.id, d.title, d.file_name, d.created_at,
       MATCH(d.title, d.description) AGAINST(:keyword IN BOOLEAN MODE) AS doc_score,
 ...
 WHERE d.tenant_id = :tenantId
   AND d.is_deleted = FALSE
   AND (
       MATCH(d.title, d.description) AGAINST(:keyword IN BOOLEAN MODE)
       OR MATCH(t.name) AGAINST(:keyword IN BOOLEAN MODE)
   )
 GROUP BY d.id
 ORDER BY (doc_score + COALESCE(tag_score, 0)) DESC
 LIMIT :size OFFSET :offset;
```
...
| tenantId | WHERE tenant_id = :tenantId | 테넌트 필터 (필수) |
...
데이터베이스 스키마 형상 관리를 위해 Flyway를 사용한다.
...
├── V2__insert_initial_tenants.sql # 초기 테넌트 데이터
└── ...                           # 이후 변경사항
```
...
| 항목 | 규칙 | 예시 |
|------|------|------|
| 접두사 | V (버전 기반 마이그레이션) | V1, V2, V3 |
| 구분자 | __ (언더스코어 2개) | V1__ |
| 설명 | snake_case 영문 | init_schema |
| 확장자 | .sql | .sql |

### 7.3 V1__init_schema.sql

초기 스키마 마이그레이션 파일은 위 [3. 테이블 스키마 상세](#3-테이블-스키마-상세)에 정의된 6개 테이블의 CREATE TABLE DDL을 순서대로 포함한다.

```sql
-- ============================================================
-- sesac-smartdocs 초기 스키마
...
### 7.4 V2__insert_initial_tenants.sql
...
- [05_네트워크_보안_설계서.md](05_네트워크_보안_설계서.md) - 네트워크/보안 설계서 (VLAN 50)
