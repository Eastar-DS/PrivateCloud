...
업로드된 문서의 메타데이터를 관리하는 핵심 테이블이다. 문서 원본 파일은 NFS 파일 스토리지에 저장되며, 이 테이블은 메타데이터만 보관한다. 텍스트 직접 입력과 파일 업로드를 모두 지원하며, `document_type`으로 구분한다.
...
| storage_key | VARCHAR(1000) | | NFS 파일 상대 경로 (파일 업로드 시) |
