...
본 문서는 sesac-smartdocs 백엔드 애플리케이션이 연동하는 외부 시스템의 프로토콜, 메시지 규격, 설정 및 장애 대응 전략을 정의한다. Spring Boot 애플리케이션을 중심으로 MariaDB, RabbitMQ, Ceph RGW, AI Worker 간의 통신 설계를 포함한다.
...
| 항목 | 내용 |
|------|------|
| 호스트 | dev-ai-worker-01 |
| 위치 | VLAN 50 (데이터망) |
| 런타임 | Python 3.10+ / Docker |
| 모델 | KoBERT (CPU 추론) |
| RabbitMQ 클라이언트 | **pika** (경량 RabbitMQ Python 클라이언트) |
| 통신 방식 | RabbitMQ (요청 수신, pika) / HTTP (결과 콜백, requests) |
| 개발 담당 | 컴퓨트/개발 팀 (백엔드 개발자) |
...
│        │ 5. (필요 시) OCR 텍스트 추출                             │
│        │                                                         │
│        │ 6. HTTP POST 콜백 (결과 반환)                            │
│        ▼                                                         │
│   [Spring Boot /internal/v1/tagging/callback]                    │
│        │                                                         │
│        │ 7. tags 테이블 INSERT                                    │
│        │ 8. tagging_jobs.status 갱신                              │
│                                                                  │
└──────────────────────────────────────────────────────────────────┘
```
...
| `boto3` | Ceph S3 문서 다운로드/업로드 |
| `requests` | Spring Boot 콜백 API 호출 |
