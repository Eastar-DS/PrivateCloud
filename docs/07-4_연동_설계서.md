...
4. [Ceph S3 오브젝트 스토리지 연동](#4-ceph-s3-오브젝트-스토리지-연동)
...
| **Ceph RGW (S3)** | S3 API | VLAN 50 (스토리지망) | 문서 원본 파일 저장 |
...
| **콜백 불필요** | Consumer가 동기적으로 AI 서버를 호출하고 직접 DB/S3에 저장하므로 콜백 API 불필요 |
| **S3 URL 전달** | AI 서버에 파일을 직접 전송하지 않고, S3 URL을 전달하여 AI 서버가 직접 다운로드 |
...
| storageKey | String | O | S3 오브젝트 키 (파일 경로) |
...
| 필드 | 타입 | 필수 | 설명 |
|------|------|------|------|
| storage_key | String | O | Ceph S3 오브젝트 키 |
...
## 4. Ceph S3 오브젝트 스토리지 연동

### 4.1 S3 연동 개요

| 항목 | 설정 |
|------|------|
| **프로토콜** | S3 API (AWS SDK 호환) |
| **엔드포인트** | `http://10.0.50.x:7480` (Ceph RGW) |
| **버킷 명명** | `sesac-smartdocs-{tenantId}` |
| **오브젝트 키** | `{tenantId}/{yyyy}/{MM}/{dd}/{documentId}.{ext}` |
| **위치** | VLAN 50 (스토리지망) |

### 4.2 버킷 구조

```
sesac-smartdocs-dept-a/          # 부서 A 전용 버킷
├── dept-a/2026/02/10/
│   ├── 7c9e6679.pdf             # 문서 원본
│   └── 7c9e6679-ocr.txt         # OCR 추출 텍스트
│
sesac-smartdocs-dept-b/          # 부서 B 전용 버킷
├── dept-b/2026/02/10/
│   └── ...
│
sesac-smartdocs-dept-c/          # 부서 C 전용 버킷
├── dept-c/2026/02/10/
│   └── ...
```
...
| 격리 수준 | 방식 | 설명 |
|----------|------|------|
| **버킷 분리** | 테넌트별 전용 버킷 | `sesac-smartdocs-dept-a`, `sesac-smartdocs-dept-b`, ... |
| **접근 제어** | Bucket Policy | 각 버킷은 해당 테넌트만 접근 가능 |
| **앱 레벨** | TenantContext | 요청 시 tenantId 기반으로 버킷 자동 선택 |
...
⑤ Consumer → AI 서버 OCR (:8001)
   ...
   │         OCR 결과 텍스트 → Ceph S3 저장 ({documentId}-ocr.txt)
   │
...
① 사용자 → POST /api/documents (title + content)
   │
② App 서버: content → .txt 파일로 변환 → Ceph S3 저장
...
public class TaggingPipelineService {
    ...
    private String resolveText(TaggingMessage message) {
        ...
        // txt 파일인 경우 S3에서 직접 읽기
        if ("txt".equals(type)) {
            return s3StorageService.readText(message.getTenantId(), message.getStorageKey());
        }
        ...
        s3StorageService.saveText(message.getTenantId(), ocrKey, ocrResult.getExtractedText());
...
| S3 장애 | 파일 저장 실패 → 업로드 자체 실패 | 사용자에게 즉시 에러 반환 |
...
| 전체 파이프라인 | 120초 | OCR + 태깅 + DB/S3 저장 전체 |
...
- [06_스토리지_설계서.md](06_스토리지_설계서.md) - 스토리지 설계서 (Ceph S3)
