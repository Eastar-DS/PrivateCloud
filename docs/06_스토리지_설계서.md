...
본 문서는 AI 플랫폼 인프라의 스토리지 아키텍처를 정의한다. Ceph 기반 분산 스토리지를 통해 테넌트 데이터 격리, 성능 티어링, 데이터 보호를 구현하며, AI 워크로드(학습/추론)를 위한 고성능 스토리지 계층을 제공한다.

> **핵심 원칙**: 테넌트는 업무 시스템(웹/앱/DB)을 운영하지 않으며, AI 서비스 이용을 위한 데이터 업로드 영역만 제공한다.
...
| **tenant-a-data** | 수원시 데이터 업로드 영역 | 3 | 64 | 100GB | Hot |
| **tenant-b-data** | 성남시 데이터 업로드 영역 | 3 | 64 | 100GB | Hot |
| **tenant-c-data** | 용인시 데이터 영역 (API 결과) | 3 | 32 | 50GB | Hot |
...
### 5.2 테넌트 데이터 영역 쿼터 설정

> **참고**: 테넌트는 VM 디스크가 아닌 데이터 업로드 영역만 사용 (AI 서비스 이용자)

```yaml
# 테넌트 데이터 영역 쿼터 (AI 플랫폼 이용자)
tenant_quotas:
  tenant-a (수원시):
    # 데이터 업로드 쿼터
    data_upload:
  ...
  tenant-c (용인시):
    # VM 없음, API 접근만
    data_upload:
      monthly_limit: 30GB  # API 직접 업로드
      storage_limit: 50GB
    api_results:
      storage_limit: 10GB
      retention: 30d
    ai_quota:
      gpu_time_monthly: 50h
...
```bash
# 테넌트 데이터 영역 Ceph 사용자 생성
# Tenant A (수원시) - 데이터 업로드 영역만
ceph auth get-or-create client.tenant-a \
  mon 'allow r' \
  osd 'allow rwx pool=tenant-a-data' \
  mds 'allow rw path=/tenant-a/upload' \
  -o /etc/ceph/ceph.client.tenant-a.keyring
# ※ VM 디스크 풀 없음 (업무 시스템 없음)

# Tenant B (성남시) - 데이터 업로드 영역만
ceph auth get-or-create client.tenant-b \
  mon 'allow r' \
  osd 'allow rwx pool=tenant-b-data' \
  mds 'allow rw path=/tenant-b/upload' \
...
| 2.0 | 2026-02-02 | AI 스토리지 아키텍처 추가: 모델 레지스트리, 데이터셋, 체크포인트, 캐시 스토리지 | - |
| 3.0 | 2026-02-04 | **AI 플랫폼 중심 재구성**: 테넌트 풀을 데이터 영역으로 변경, VM 디스크 쿼터를 데이터 업로드 쿼터로 변경, Ceph 인증에서 VM 풀 접근 제거 | - |
