...
| **MDS** | 2 | CephFS 메타데이터 및 NFS 게이트웨이 관리 | 파일 스토리지용 (NFS 기반 문서 저장 포함) |
...
| ~~**search-index**~~ | ~~Elasticsearch~~ | - | - | - | - (제거됨, MariaDB 기본 검색 사용) |
...
#### 3.3.2 파일 스토리지 (CephFS + NFS)

공유 파일 시스템, 문서 저장 (NFS를 통해 백엔드 서버에 마운트)
...
cephfs:
  ...
  # NFS 게이트웨이 설정 (Ceph NFS-Ganesha)
  nfs_gateway:
    bind_addr: 10.0.50.10
    port: 2049
    protocols: [4.1]
    exports:
      - path: /documents
...
```bash
# 백엔드 서버 NFS 마운트 설정
mount -t nfs -o vers=4.1,soft,timeo=30,retrans=3 \
  10.0.50.10:/documents /mnt/documents

mount -t nfs -o vers=4.1,soft,timeo=30,retrans=3,ro \
  10.0.50.10:/models /mnt/models
```

#### 3.3.3 NFS 파일 스토리지 (CephFS + NFS 게이트웨이)

> **참고**: 오브젝트 스토리지(RGW)는 본 프로젝트에서는 사용하지 않음. 문서 파일, 태깅 결과, 모델은 CephFS 기반 NFS 파일 스토리지를 통해 저장한다.

```yaml
# NFS 파일 스토리지 설정 (CephFS 백엔드)
nfs_file_storage:
  protocol: NFS v4.1
  port: 2049
  backend: CephFS
  ...
  # 문서 태깅 서비스 디렉토리
  directories:
    - path: /mnt/documents/dept-a
      quota: 100GB
      description: "부서 A 문서 원본"

    - path: /mnt/documents/dept-b
      quota: 100GB
      description: "부서 B 문서 원본"

    - path: /mnt/documents/dept-c
      quota: 100GB
      description: "부서 C 문서 원본"

    - path: /mnt/models
      quota: 100GB
      description: "태깅 모델 파일"

    - path: /mnt/documents/tagging-results
      quota: 200GB
      lifecycle:
        - transition_days: 90
          storage_class: WARM
...
document_storage:
  ...
  capacity:
    ...
    total: 300GB
    file_size_limit: 20MB   # 단일 파일 최대 크기
```
...
tagging_result_storage:
  ...
  result_format:
    ...
    detail_storage:
      format: json
      location: /mnt/documents/tagging-results/{dept}/{doc_id}/result.json
      content:
        - raw_predictions
        - embeddings (optional)
        - processing_log
...
document_processing_flow:
  ...
  upload:
    source: API Server
    destination: /mnt/documents/{dept}/uploads/{date}/{file_id}/
    actions:
      - validate_file_type
      - calculate_checksum
      - store_metadata
  ...
  # 3. 태깅 처리 단계
  processing:
    source: /mnt/documents/{dept}/uploads/
  ...
  # 4. 결과 저장 단계
  complete:
    document_dest: /mnt/documents/{dept}/processed/{date}/{file_id}/
    result_dest: /mnt/documents/tagging-results/{dept}/{doc_id}/
    actions:
      - move_document
      - store_result
      - update_search_index
  ...
  # 5. 실패 처리
  failure:
    destination: /mnt/documents/{dept}/failed/{date}/{file_id}/
    actions:
      - store_error_log
      - notify_admin
```
...
| **RGW** | RADOS Gateway, S3 호환 오브젝트 스토리지 (본 프로젝트에서는 사용하지 않음) |
...
- NFS-Ganesha Documentation: https://nfs-ganesha.github.io
...
| 5.1 | 2026-02-09 | PostgreSQL→MariaDB 변경, Redis 참조 제거, sesac-smartdocs 반영 | - |
| 6.0 | 2026-02-10 | 아키텍처 변경 반영: VLAN 번호 업데이트 (스토리지 40→50, 백엔드 30→20), 파일 크기 100MB→20MB, 태깅 워커→AMQP Consumer | - |
| 7.0 | 2026-02-11 | S3/RGW 참조를 NFS(CephFS)로 전면 변경: RGW 제거, MDS 역할 확대, 오브젝트 스토리지→NFS 파일 스토리지, 버킷→디렉토리, s3:// 경로→/mnt/documents/ 경로, NFS 마운트 설정 추가 | - |
