...
|-------------|-----------|---------------|-----------|----------|--------|
| POST | `/v1/documents` | O | O | O | X |
| GET | `/v1/documents/{id}` | O | O | O | O |
| GET | `/v1/documents/{id}/download` | O | O | O | O |
| DELETE | `/v1/documents/{id}` | O | O | X | X |
| PUT | `/v1/documents/{id}/tags` | O | O | X | X |
| DELETE | `/v1/documents/{id}/tags/{tagId}` | O | O | X | X |
| GET | `/v1/search` | O | O | O | O |
| POST | `/v1/tenants` | O | X | X | X |
| PUT | `/v1/tenants/{id}` | O | X | X | X |
| GET | `/v1/audit-logs` | O | O | X | X |
...
| 계층 | 격리 방식 | 설명 |
|------|----------|------|
| 네트워크 | VLAN | 테넌트별 전용 VLAN (10, 20, 30) |
| 스토리지 | Ceph Pool | 테넌트별 전용 Pool 분리 |
| 데이터베이스 | Row-Level 필터링 | Hibernate `@Filter`로 자동 WHERE 조건 추가 |
| API | TenantContext | 모든 요청에 테넌트 ID 필수 포함 |
...
[nginx - TLS Termination]
      │
      │  HTTP (VLAN 40 내부, 네트워크 격리)
      ▼
[Spring Boot App 서버]
      │
      ├──── HTTP ────► [AI 태깅 워커] (VLAN 40 내부)
      ├──── TCP 3306 ─► [MariaDB] (VLAN 50, SSL 선택적)
      └──── S3/NFS ───► [Ceph] (VLAN 60, 내부망)
```

| 구간 | 프로토콜 | 암호화 | 비고 |
|------|---------|--------|------|
| 사용자 - nginx | TLS 1.3 | AES-256-GCM | 외부 접근 구간, HSTS 적용 |
| nginx - App 서버 | HTTP | 없음 (VLAN 40 격리) | 내부 서비스망, 네트워크 격리로 보호 |
| App 서버 - MariaDB | TCP 3306 | SSL 선택적 | 데이터망 VLAN 50 내부 |
| App 서버 - RabbitMQ | TCP 5672 | 없음 (VLAN 내부) | 내부 통신 |
| App 서버 - Ceph | S3/NFS | 없음 (VLAN 60 격리) | 스토리지 전용 네트워크 |
...
| Ceph 접근 키 | 환경 변수 (`CEPH_ACCESS_KEY`, `CEPH_SECRET_KEY`) |
...
| 검증 항목 | 정책 | 비고 |
|----------|------|------|
| 허용 확장자 | PDF, DOCX, DOC, XLSX, XLS, PPTX, PPT, TXT, RTF, HWP, JPG, JPEG, PNG, TIFF, BMP | 화이트리스트 방식 |
| 최대 파일 크기 | 100MB | `spring.servlet.multipart.max-file-size=100MB` |
...
public class FileValidator {
    ...
    private static final Set<String> ALLOWED_EXTENSIONS = Set.of(
        ...
        "txt", "rtf", "hwp", "jpg", "jpeg", "png", "tiff", "bmp"
    );
    ...
    public void validate(MultipartFile file) {
        ...
        if (file.getSize() > MAX_FILE_SIZE) {
            throw new FileValidationException("파일 크기가 100MB를 초과합니다");
        }
...
테넌트(부서)별로 API 호출 빈도를 제한하여 특정 테넌트의 과도한 사용이 다른 테넌트에 영향을 주는 것을 방지한다.

| 엔드포인트 | 제한 | 단위 | 비고 |
|-----------|------|------|------|
| `POST /v1/documents` (업로드) | 100 req/min | 테넌트별 | 파일 업로드 + AI 태깅 부하 고려 |
| `GET /v1/search` (검색) | 500 req/min | 테넌트별 | 검색 쿼리 부하 고려 |
| 기타 엔드포인트 | 1,000 req/min | 테넌트별 | 일반 API 호출 |
...
| 전파 범위 | nginx - App 서버 - AI 워커 - DB - 감사 로그 |
