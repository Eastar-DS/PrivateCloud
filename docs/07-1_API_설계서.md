...
|---|--------|--------|-----|------|----------|
| 1 | Document | POST | `/api/documents` | 문서 업로드 (텍스트/파일, 사전 태깅 지원) | dept-user |
...
**사전 태깅 모드**: `tags` 파라미터를 함께 전달하면 AI 태깅 파이프라인을 건너뛰고 태그를 즉시 저장한다. 이 경우 Job은 생성되지 않으며, 문서의 `taggingStatus`는 즉시 `COMPLETED`가 된다. 사전에 태깅된 학습 데이터를 일괄 업로드하는 용도로 사용한다.
...
| tags | String (JSON Array) | X | 사전 태깅 태그 목록 (예: `["예산","계획서"]`). 전달 시 AI 태깅을 건너뛴다 |
...
| description | String | X | 문서 설명 (최대 1000자) |
| tags | List\<String\> | X | 사전 태깅 태그 목록. 전달 시 AI 태깅을 건너뛴다 |

#### 서버 동작 분기

| 조건 | taggingStatus | Job 생성 | RabbitMQ 발행 | 태그 저장 |
|------|-------------|---------|-------------|---------|
| `tags` **미전달** | `PENDING` | O (자동) | O | AI 파이프라인 완료 후 저장 (`source=AI`) |
| `tags` **전달** | `COMPLETED` | X | X | 즉시 저장 (`source=IMPORT`, `confidence=1.0`) |

#### Response (201 Created) — tags 미전달 시
...
#### Response (201 Created) — tags 전달 시 (사전 태깅)
